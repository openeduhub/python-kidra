...
This ~Python~ service serves as a unified API to
...
Now, simply load the created image through
#+begin_src sh
docker load < result
#+end_src
or
...
The input data, and potential parameters, are passed through ~JSON~ objects.

** Text Statistics

Various statistics on a given piece of text, or text from a ~URL~, can be processed through the =analyze-text= sub-domain.
